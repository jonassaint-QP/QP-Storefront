...
export const SUPPORTED_SUBSCRIPTION_COUNTRIES = ['US', 'CA'] as const;  
export type SubscriptionCountry = (typeof SUPPORTED_SUBSCRIPTION_COUNTRIES)[number];

// Canada stays disabled until fulfillment, tax, shipping, and payment approval are complete.  
export const ENABLED_SUBSCRIPTION_COUNTRIES = ['US'] as const satisfies readonly SubscriptionCountry[];
...
export function validateSubscriptionEnrollment(  
 ...
 if (!input.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email)) {  
  ...
  if (!(ENABLED_SUBSCRIPTION_COUNTRIES as readonly SubscriptionCountry[]).includes(input.country as SubscriptionCountry)) {
   return { ok: false, error: 'The Sovereign Body Lube Club is not available in that country at launch.' };  
 }
